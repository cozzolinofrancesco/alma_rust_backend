...
import type { StepNodeData } from '../hooks/useDiagramModel';

function StepNodeImpl({ data }: NodeProps<StepNodeData>) {
  const { t } = useLanguage();
  const tagColors = data.tag ? getTagColor(data.tag) : null;

  const className = [
    'c272-node',
    data.isEdited ? 'c272-node--edited' : '',
  ]
    .filter(Boolean)
    .join(' ');
  ...
  const handleColor = tagColors?.border ?? '#c0bbeb';
  ...
  return (
    <>
      ...
      <div
        ...
        role="button"
        tabIndex={0}
        onClick={data.onClick ? () => data.onClick!(data.layerId) : undefined}
        onKeyDown={(e) => {
          if (data.onClick && (e.key === 'Enter' || e.key === ' ')) {
            e.preventDefault();
            data.onClick(data.layerId);
          }
        }}
      >
        <div className="c272-node__index" style={tagColors ? { color: tagColors.text } : undefined}>
          {data.indexLabel}
        </div>
        {data.corpusId ? (
          <span
        ...
        <div className="c272-node__title" title={data.title}>
          {data.title}
        </div>
      </div>
      {}
...
export default memo(StepNodeImpl);
